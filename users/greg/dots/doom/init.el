...
(doom! :input
       ...
       zen                 ; distraction-free writing: writeroom + mixed-pitch (SPC t z)
       ...
       direnv              ; per-buffer env → eglot uses each project's devShell
       docker
       ...
       (lua +lsp
            +treesitter)  ; one-based indices? one-based indices
       markdown            ; writing docs for people to ignore
       ;;nim               ; python + lisp at the speed of c
       (nix +lsp
            +treesitter)   ; nix language + nixd LSP
       ;;ocaml             ; an objective camel
       ;;odin              ; C, minus its footguns
       (org +roam2)      ; org + org-roam: networked/linked notes (Obsidian-style)
       ...
       (web +lsp
            +treesitter)  ; the tubes — HTML + CSS (vscode-html/css LSPs)
